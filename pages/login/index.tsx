  ...
  function LoginFormDesktop() {
    ...
    const handleLogin = async (event: FormEvent) => {
      ...
      try {
        ...
        if (signInError) {
          console.error('ERROR (Login Form): Error de inicio de sesión de Supabase:', signInError);
          if (signInError.message === 'Invalid login credentials') {
            setError('Correo electrónico o contraseña incorrectos. Por favor, inténtalo de nuevo.');
          } else if (signInError.message.includes('Email not confirmed')) {
            setError('Tu cuenta no ha sido confirmada. Revisa tu correo electrónico para el enlace de verificación.');
          } else {
        ...
        if (!data.user || !data.session) {
          setMessage('Revisa tu correo electrónico para confirmar tu cuenta y luego intenta iniciar sesión de nuevo.');
          setLoading(false);
          return;
        }
        ...
        console.error('ERROR (Login Form): Error inesperado:', err);
